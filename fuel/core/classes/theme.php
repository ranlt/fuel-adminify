...
class Theme
{
	...
	public function view($view, $data = array(), $auto_filter = null)
	{
		...
		\Log::debug("theme view: ".$this->find_file($view));
	...
	protected function find_file($view, $themes = null)
	{
		...
		foreach ($themes as $theme)
		{
			...
			if (empty($theme['find_file']))
			{
				...
				{
					\Log::debug("1".$theme['path'].$path_prefix.$file.$ext);
					return $path;
				}
				elseif (is_file($path = $theme['path'].$file.$ext))
				{
					\Log::debug("2".$theme['path'].$file.$ext);
					return $path;
				} 
				elseif ($module_path and ! empty($theme['name']) and is_file($path = $module_path.$theme['name'].DS.$file.$ext))
				{
					\Log::debug("3".$module_path.$theme['name'].DS.$file.$ext);
					return $path;
				} else {
					\Log::debug("4".$module_path.$theme['name'].DS.$file.$ext);
				}
			}
			else
			{
